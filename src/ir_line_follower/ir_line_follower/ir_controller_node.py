    ...
    def ir_callback(self, msg):
        if len(msg.data) != 3:
            self.get_logger().warn(
                f'Expected 3 IR sensor values, got {len(msg.data)}',
        ...
        if (left, center, right) == (0, 1, 0):
            ...
            self.last_turn_dir = 1.0
        elif (left, center, right) == (1, 0, 0):
            cmd.linear.x = self.v_lin * 0.4
            cmd.angular.z = self.v_hard
            self.last_turn_dir = 1.0
        elif (left, center, right) == (0, 1, 1):
            cmd.linear.x = self.v_lin * 0.7
            cmd.angular.z = -self.v_soft
            self.last_turn_dir = -1.0
        elif (left, center, right) == (0, 0, 1):
            cmd.linear.x = self.v_lin * 0.4
            cmd.angular.z = -self.v_hard
            self.last_turn_dir = -1.0
        elif (left, center, right) == (1, 1, 1):
            cmd.linear.x = self.v_lin
            cmd.angular.z = 0.0
        elif (left, center, right) == (1, 0, 1):
            cmd.linear.x = self.v_lin
            cmd.angular.z = 0.0
        else:
            cmd.linear.x = 0.0
